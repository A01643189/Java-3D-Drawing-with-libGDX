...
public class Drawing3D extends ApplicationAdapter {
    ...
    public void create() {
        ...
        // Setup model batch and environment 
        ...
        // Create model instances and position them in space 
    ...
    private void handleInput() { 
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.A)) camera.position.add(camera.direction.cpy().crs(camera.up).nor().scl(-delta));             if (Gdx.input.isKeyPressed(Input.Keys.D)) camera.position.add(camera.direction.cpy().crs(camera.up).nor().scl(delta)); 
        // Camera rotation 
